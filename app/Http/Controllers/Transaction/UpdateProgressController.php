...
{
    const URL       = 'transaction/update-progress';
    const RESOURCE  = 'Transaction\Update Progress';
    protected $now;
    ...
    public function index(Request $request){
    	...
    	$query = \DB::table('iapsys.project_progress')
        			->select(
                        ...
                        'project_progress.validated_by',
                        'project_progress.validated_date',
                    ...
                    ->orderBy('project_progress.project_progress_id', 'desc')
                    ->groupBy(
                        'project_progress.project_progress_id',
                        'project_progress.created_date',
                        'project_progress.created_by',
        ...
        if (!empty($filters['validated'])) {
            if($filters['validated'] == "Valid"){
                $query->whereNotNull('project_progress.validated_date');
            }else{
                $query->whereNull('project_progress.validated_date');
            }
        }
        ...
        return view('transaction.update-progress.index', [
    ...
    public function update(Request $request, $id){
        ...
        return view('transaction.update-progress.update', [
            'url'     => self::URL,
            'model'   => $model,
        ]);
    ...
    public function view(Request $request, $id){
        ...
        return view('transaction.update-progress.view', [
            'url'     => self::URL,
            'model'   => $model,
        ]);
    ...
    public function delete(Request $request){
        ...
        $model = ProjectProgress::find($request->get('projectProgressId'));

        if(!empty($model->validated_date)){
            abort(404);
        }

        foreach ($model->milestoneProgress as $milestoneProgress) {
        ...
        $query   = $this->getProject($model->project_id);
        $project = $query->first();

        $request->session()->flash(
            'successMessage',
            'Progress project '.$project->description.' successfully removed'
        );
    ...
    public function save(Request $request){
        ...
        $modelProject->last_updated_by      = \Session::get('user')->vc_username;
        $modelProject->last_updated_date    = $now->format('Y-m-d H:i:s');
        ...
        foreach ($modelProject->milestoneProgress as $milestoneProgress) {
            ...
            $milestoneProgress->last_updated_by        = \Session::get('user')->vc_username;
            $milestoneProgress->last_updated_date      = $now->format('Y-m-d H:i:s');
            ...
            foreach ($milestoneProgress->taskProgress as $taskProgress) {
                ...
                $taskProgress->last_updated_by      = \Session::get('user')->vc_username;
                $taskProgress->last_updated_date    = $now->format('Y-m-d H:i:s');
        ...
        $request->session()->flash(
            'successMessage',
            'Progress project '.$project->description.' successfully updated'
        );
