...
@section('title', 'Detail History Milestone')
  ...
  <div class="col-md-12">
    <div class="widget">
        <header class="widget-header">
            <h4 class="widget-title">Detail History Milestone</h4>
        </header>
        <hr class="widget-separator">
        <div class="widget-body">
            <form class="form-horizontal" method="POST" action="">
            {{ csrf_field() }}
                <input type="hidden" name="milestoneId" value="138">
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="select2-demo-1" class="col-sm-4 control-label">Project Name</label>
                            <div class="col-sm-8">
                                <select class="form-control" data-plugin="select2" name ="projectId" id="projectId">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-8">
                                <select class="form-control" data-plugin="select2" name ="sectionDiv" id="sectionDiv">
                                    <option value="">Please Select</option>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-8">
                                <select class="form-control" data-plugin="select2" name ="milestone" id="milestone">
                                    <option value="">Please Select</option>
                                    @foreach($milestonesOption as $milestone)
                                    <option value="{{ $milestone->milestone_id }}" {{ !empty($filters['milestone']) && $filters['milestone'] == $milestone->milestone_id ? 'selected' : '' }}>{{ $milestone->milestone_name }}</option>
                                    @endforeach
                                </select>
                            </div><!-- END column -->
                        </div><!-- .form-group -->
...
<script type="text/javascript">
    $(document).on('ready', function() {
        $('#projectId').on("select2:select", updateFormMilestone);
        $('#sectionDiv').on('select2:select', updateFormMilestone)
    })

    var updateFormMilestone = function() {
        var projectId = $('#projectId').val();
        var sectionDiv = $('#sectionDiv').val();

        $.ajax({
            method: "POST",
            url: "{{ URL($url.'/get-milestone') }}",
            data: {projectId: projectId, sectionDiv: sectionDiv, _token: "{{ csrf_token() }}"}
        }).done(function(result) {
            $('#milestone').html('');
            $('#milestone').append('<option value="">Please Select</option>');
            result.forEach(function(milestone) {
                $('#milestone').append('<option value="' + milestone.milestone_id + '">' + milestone.milestone_name + '</option>');
            })
        });
    };
</script>
